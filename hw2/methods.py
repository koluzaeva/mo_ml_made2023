...
class LineSearchTool(object):
    ...
    def line_search(self, oracle, x_k, d_k, previous_alpha=None):
        ...
        phi_grad = lambda alpha: oracle.grad_directional(x_k, d_k, alpha)
        phi0 = phi(0)
        ...
        if self._method == 'Constant':
            ...
            while  phi(previous_alpha) >  phi0 + self.c1 * previous_alpha * phi_grad0:
              previous_alpha /= 2
            return previous_alpha
              
        elif self._method == 'Wolfe':
            # your code here
            res = scalar_search_wolfe2(phi, phi_grad, phi0,phi_grad0, c1 = self.c1, c2 = self.c2)[0]
            if res is None:
                self._method = 'Armijo'
                return self.line_search(oracle, x_k, d_k, 1.0)
            else:
              return res
...
class GradientDescent(object):
    ...
    def run(self, max_iter=10000):
        ...
        stop_fun = lambda x: np.dot(self.oracle.grad(x), self.oracle.grad(x).T)
        norm_grad_x0 = np.dot(self.oracle.grad(self.x_0), self.oracle.grad(self.x_0).T)
        x_k = self.x_0.copy()
        ...
        if norm_grad_x0 != 0:
          while i < max_iter and stop_fun(x_k) / norm_grad_x0 > self.tolerance:
            d_k = - self.oracle.grad(x_k)
            alpha_k = self.line_search_tool.line_search(self.oracle, x_k, d_k, alpha_k)
            x_k = x_k + d_k * alpha_k
            i += 1
